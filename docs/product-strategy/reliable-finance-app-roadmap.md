...
- Transaction durability: Home and Activity now sync transaction snapshots to the Cloudflare Worker with Clerk bearer-token ownership, visible sync/fallback state, dedicated transaction KV namespaces, transaction sync metrics, and MMKV retained as a local cache/fallback.
