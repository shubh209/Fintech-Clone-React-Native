...
- Transaction durability: Home and Activity now sync transaction snapshots to the Cloudflare Worker with Clerk bearer-token ownership, visible sync/fallback state, and MMKV retained as a local cache/fallback. A dedicated transaction KV namespace remains a production-hardening follow-up blocked on Cloudflare namespace-create permissions.
