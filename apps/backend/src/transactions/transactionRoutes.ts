...
import { getAuthenticatedTransactionUserId } from './transactionAuth';
...
transactionRoutes.get('/transactions', async (context) => {
  const userId = await getAuthenticatedTransactionUserId({
    authorization: context.req.header('authorization'),
    env: context.env,
  });

  if (!userId) {
    return context.json({ error: 'Missing or invalid transaction bearer token' }, 401);
  }
